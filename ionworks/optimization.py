...
class OptimizationClient:
    ...
    def run(self, data: dict[str, Any]) -> Optimization:
        ...
        response_data = self.client.post("/optimize/run", data)
        return Optimization(**response_data)

    def get(self, optimization_id: str) -> dict[str, Any]:
        """Get an optimization with its associated job status.
        ...
        -------
        dict[str, Any]
            Dictionary with ``optimization`` and ``job`` keys.
        """
        return self.client.get(f"/optimize/{optimization_id}")
    ...
    def list(
        ...
        -------
        dict[str, Any]
            Dictionary with ``jobs``, ``optimizations``, and ``total`` keys.
        """
    ...
    def update(self, optimization_id: str, data: dict[str, Any]) -> Optimization:
        ...
        """
        response_data = self.client.patch(f"/optimize/{optimization_id}", data)
        return Optimization(**response_data)
    ...
    def cancel(self, optimization_id: str) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            The job response with updated status.
        """
        return self.client.post(f"/optimize/{optimization_id}/cancel", {})
    ...
    def wait_for_completion(
        ...
        -------
        dict[str, Any]
            The final optimization response (with ``optimization`` and ``job``
            keys).
        ...
        while True:
            ...
            if status in terminal_states:
                if status != "completed" and raise_on_failure:
                    error_msg = result["job"].get("error", "Unknown error")
                    raise IonworksError(
                        {
