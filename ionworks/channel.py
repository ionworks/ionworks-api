...
class ChannelClient:
    ...
    def update(
        ...
        data : dict[str, Any]
            ...
            ``service_scheduled_until`` (ISO datetime) records when that service
            is expected to end, making the outage a span rather than a start
            instant — it is what lets the lab say when the channel is back.
            Optional even for a planned visit, but must be after
            ``service_scheduled_at``. Sending it on its own books an end against
            a visit already scheduled; the existing start is kept.
        ...
        IonworksError
            ...
            ``service_scheduled_at``, or if ``service_scheduled_until`` is not
            after the visit's start.
        ...
        Take a channel out of service for scheduled maintenance::
            ...
            client.channel.update(
                channel_id,
                {
                    ...
                    "service_scheduled_until": "2026-09-03T17:00:00Z",
                },
            )
        """
        endpoint = f"/channels/{channel_id}"
    ...
    def list_incidents(
        ...
        limit: int | None = None,
        offset: int | None = None,
    ) -> PaginatedList[ChannelIncident]:
        """List a channel's out-of-service history, most recent outage first.

        The read side of :meth:`update`: taking a channel out of commission and
        returning it records a span, and this is how those spans are read back.
        ``out_of_commission`` on the channel itself only says whether it is out
        *now* — it carries no cause, no booked service, and no history.

        Both kinds of outage appear here. An incident with ``service_event_id``
        set was opened by an instrument-level visit on the parent cycler (see
        :meth:`CyclerClient.start_service`), so a calibration shows up once per
        channel it took down; one with ``service_event_id`` None is a fault the
        channel had on its own.

        Parameters
        ----------
        channel_id : str
            The channel whose history to read.
        limit : int | None, optional
            Page size. The API defaults to 20 and caps it at 100.
        offset : int | None, optional
            Rows to skip.
        ...
        -------
        PaginatedList[ChannelIncident]
            One page of outages, newest first, with ``.total`` for the full
            count. The one with ``resolved_at is None``, if any, is the
            channel's current outage; at most one can be open.

        Examples
        --------
        Find out why a channel is down and when it is expected back::

            incidents = client.channel.list_incidents(channel_id)
            current = next((i for i in incidents if i.resolved_at is None), None)
            if current is not None:
                print(current.category, current.service_scheduled_until)
                if current.is_overdue_to_return:
                    print("Overdue — booked to be back already.")

        Note that ``is_estimated`` rows were reconstructed from the channel's
        last-modified time when outage history was introduced, so their
        ``started_at`` is an upper bound; exclude them from downtime
        arithmetic::

            real = [i for i in incidents if not i.is_estimated]
        """
        endpoint = _build_endpoint(
            f"/channels/{channel_id}/incidents",
            {"limit": limit, "offset": offset},
        )
        response_data = self.client.get(endpoint)
        return _parse_list_response(response_data, ChannelIncident)
