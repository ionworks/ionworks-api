...
class FitResults(BaseModel):
    ...
    capacity_Ah: list[float] | None = Field(
        default=None,
        description=(
            "Cell capacity used to integrate SOC, one entry per measurement "
            "segment. An estimated or fitted capacity is a single cell-wide "
            "value repeated across segments (a separate capacity is not "
            "estimated per trace); entries differ only when a per-measurement "
            "``capacity`` is supplied — the list type is the same either way. "
            "Persisted on save and reused when re-simulating for validation."
        ),
    )
    ...
    def to_api_payload(self) -> dict[str, Any]:
        """Serialise the fitted curves for the save / validate request bodies."""
        return {
            "soc": self.soc,
            "ocv": self.ocv,
            "r0": self.r0,
            "rc_pairs": [rc.model_dump() for rc in self.rc_pairs],
...
class ValidationResults(BaseModel):
    """Held-out validation result from :py:meth:`ECMClient.validate`.

    Error metrics are in millivolts; the trace arrays are downsampled and
    aligned for a model-vs-data overlay (``time``/``data_voltage``/
    ``model_voltage``) plus a residual panel (``residual_mV``).
    """

    rmse_mV: float = Field(description="Root-mean-square voltage error [mV].")
    mae_mV: float = Field(description="Mean absolute voltage error [mV].")
    max_mV: float = Field(description="Maximum absolute voltage error [mV].")
    num_rcs: int = Field(description="RC-pair count of the validated model.")
    capacity_Ah: list[float] = Field(
        description="Capacity used for SOC integration [Ah] (single-element list)."
    )
    initial_soc: float = Field(description="Initial SOC used for the held-out trace.")
    model_source: str = Field(description="Where the validated model came from.")
    time: list[float] = Field(description="Downsampled time grid [s].")
    data_voltage: list[float] = Field(description="Held-out measured voltage [V].")
    model_voltage: list[float] = Field(description="Re-simulated model voltage [V].")
    residual_mV: list[float] = Field(description="Model − data voltage residual [mV].")
...
class ECMClient:
    ...
    def fit_from_measurements(self, config: dict[str, Any]) -> EcmFitJob:
        ...
        config : dict[str, Any]
            ...
            ``end_step``, ``initial_soc``, ``capacity``; per-measurement
            ``capacity`` is all-or-none across measurements). Optional
            ``ecm_options`` with shared hyperparameters (``num_rcs``,
            ``fit_ocv``, ``capacity``, ``ocv_soc_curve``, ``bounds_capacity``,
            ``num_knots``, ``num_knots_r0``, ``knot_schedule``,
            ``regularization``, ``clamp_boundary_knots``, ``clamp_max_ratio``).
    ...
    def fit_from_file(
        ...
        ``regularization`` sets the smoothness-prior strength on the R0/RC
        curves (scale = 5 / regularization; 0 disables it, the default).
        """
    ...
    def save_to_project(
        ...
        body = {
            ...
            # The payload carries capacity_Ah so the saved model reproduces its
            # fit (its OCV/R0 curves are referenced to that capacity).
            "fit_results": fr.to_api_payload(),
        }
        response = self.client.post("/ecm/save-to-project", json_payload=body)
        return SaveToProjectResponse(**response)

    # ------------------------------------------------------------------
    # Held-out validation (synchronous, no job, no pipeline)
    # ------------------------------------------------------------------
    ...
    def validate(
        ...
        *,
        ...
        fit_results: FitResults | dict[str, Any] | None = None,
        ...
        """Validate a fitted ECM on held-out data (synchronous, no pipeline).

        Re-simulates the fitted model forward on a held-out current trace using
        the same engine ``fit_*`` runs internally, then returns aligned
        model-vs-data traces and RMSE/MAE/Max voltage error — ready to plot as a
        two-panel (overlay + residual) figure. No fit and no validation pipeline
        is involved.

        Provide exactly one held-out source (``measurement_id`` or
        ...
        measurement_id : str, optional
            Held-out cell measurement to validate against. Mutually exclusive
            with ``example_id``.
        example_id : str, optional
            Built-in example dataset id to validate against.
        fit_results : FitResults or dict, optional
            In-memory fit result (e.g. from :py:meth:`wait_for_completion`) to
            re-simulate. Mutually exclusive with ``parameterized_model_id``.
        parameterized_model_id : str, optional
            A saved ECM parameterized model to re-simulate (e.g. one returned by
            :py:meth:`save_to_project`).
        start_step, end_step : int, optional
            Inclusive step bounds applied to the held-out measurement.
        ...
        -------
        ValidationResults
            Error metrics plus the aligned overlay and residual traces.
        """
        body: dict[str, Any] = {}
        if measurement_id is not None:
            body["measurement_id"] = measurement_id
        if example_id is not None:
            body["example_id"] = example_id
        if start_step is not None:
            body["start_step"] = start_step
        if end_step is not None:
            body["end_step"] = end_step
        if initial_soc is not None:
            body["initial_soc"] = initial_soc
        if capacity is not None:
            body["capacity"] = capacity
        if parameterized_model_id is not None:
            body["parameterized_model_id"] = parameterized_model_id
        if fit_results is not None:
            ...
            body["fit_results"] = fr.to_api_payload()

        response = self.client.post("/ecm/validate", json_payload=body)
