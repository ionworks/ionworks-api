...
from ._project_id import inject_project_id, resolve_env_project_id, resolve_project_id
...
class PipelineConfig(BaseModel):
    ...
    def set_defaults(self) -> "PipelineConfig":
        """Resolve project_id from env if not provided, and set defaults."""
...
class PipelineClient:
    ...
    def create(self, config: dict[str, Any]) -> PipelineSubmissionResponse:
        ...
        """
        try:
            # Inject the client's default project_id if the config doesn't
            # specify one, so PipelineConfig's env-var fallback only runs
            # when the client also has no default.
            config = inject_project_id(self.client, config)
