...
class PipelineClient:
    ...
    def wait_for_completion(
        ...
        raise_on_failure : bool, optional
            Whether to raise IonworksError when the pipeline fails or is
            canceled (default: True).

        Returns
        -------
        PipelineSubmissionResponse
            The terminal pipeline response (failed / canceled included when
            raise_on_failure=False).

        Raises
        ------
        TimeoutError
            If timeout is reached before the pipeline reaches a terminal state.
        IonworksError
            If the pipeline failed or was canceled and raise_on_failure is True.
        """
        ...
        if pipeline.is_failed and raise_on_failure:
            error_msg = f"Pipeline {pipeline_id} {pipeline.status}"
            if pipeline.error:
