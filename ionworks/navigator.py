...
from .models import collect_all_pages
...
class Navigator:
    ...
    project_id : str, optional
        Project whose specs the navigator walks. Defaults to the project
        configured on the client (``IONWORKS_PROJECT_ID`` or the ``project_id``
        constructor argument). Spec names are unique per project, not per
        organization, so the name-keyed methods (:meth:`specs`, :meth:`spec`,
        :meth:`instances`) require one and raise :class:`ValueError` if neither
        this argument nor a client default is set. Navigating by id
        (:meth:`measurements`, :meth:`steps`, :meth:`time_series`) needs no
        project, so a navigator without one is still usable for those.
    ...
    def __init__(
        ...
        *,
        ...
        # Resolved leniently here and required only by the name-keyed methods
        # (see _require_project_id). The ID-keyed walk — measurements(),
        # steps(), time_series() — never consults a project, so a client
        # without one must still be able to build a navigator and use them.
        self.project_id = resolve_project_id(client, project_id, required=False)
    ...
    def specs(self) -> dict[str, CellSpecification]:
        """Return the navigator's project's cell specifications, keyed by name.

        Scoped to :attr:`project_id`, which is what makes keying by name
        lossless: names are unique within a project but not across the
        organization, so an org-wide listing would let a sibling project's
        spec overwrite this project's entry.
        ...
        if self._specs is None:
            self._specs = {
                s.name: s
                for s in self.client.cell_spec.list(
    ...
    def _require_project_id(self) -> str:
        """Return the navigator's project, raising if it has none.

        Called by the name-keyed methods only. Names are unique per project,
        so keying specs by name across an organization would let a sibling
        project's spec overwrite this one's.
        """
        if not self.project_id:
            raise ValueError(
                "project_id is required to look up specs by name. Pass it to "
                "Navigator(project_id=...), set it on the Ionworks client, or "
                "set the IONWORKS_PROJECT_ID environment variable. Navigating "
                "by id (measurements, steps, time_series) needs no project."
            )
    ...
    def spec(self, name: str) -> CellSpecification:
        ...
        ------
        KeyError
            If no spec with the given name exists in the navigator's project.
            The message lists the available spec names to make typos easy to
            spot.
        """
        specs = self.specs()
        if name not in specs:
            raise KeyError(
                f"No cell specification named {name!r} in project "
                f"{self.project_id!r}. Available: {sorted(specs)}"
            )
        return specs[name]
    ...
    def _collect_all_pages(self, fetch_page: Callable[[int, int], Any]) -> list[Any]:
        ...
        return collect_all_pages(fetch_page, self.page_size)
