...
class CellMeasurementClient:
    ...
    def update(self, measurement_id: str, data: dict[str, Any]) -> CellMeasurement:
        ...
        data : dict[str, Any]
            ...
            attaching/detaching a ``channel_id`` (pass ``None`` to detach) and
            for setting ``end_time`` to mark a run complete.
    ...
    def _upload_to_signed_url(
        ...
        IonworksError
            If the upload fails.
        """
        upload_to_signed_url(signed_url, data, self.UPLOAD_TIMEOUT, content_type)
    ...
    def wait_for_processing(
        ...
        Polls ``processing_status`` until every measurement reaches a terminal
        state. Steps are excluded from the reads: requesting them is exactly
        what the server refuses while processing is in flight.

        Pass every id of a batch in one call rather than waiting on each in
        turn. The server processes uploads concurrently, so a batch takes about
        as long as its slowest member — and *timeout* is one deadline shared
        across the batch, where per-measurement waits would each carry their
        own and a stuck batch could hang for a multiple of it.

        Waiting is not fail-fast: a measurement the server rejects does not
        stop the others being waited on. Every failure in the batch is
        reported together, once, when the batch settles.
    ...
    def create(
        ...
        parquet_bytes = dataframe_to_parquet(time_series)
        ...
        if steps is not None:
            ...
            steps_parquet = dataframe_to_parquet(steps_pl)
