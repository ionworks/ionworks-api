...
from ._dataframe import parquet_to_dataframe, parquet_upload_body, to_polars
from ._uploads import upload_file
...
#: Worker threads used to upload a measurement's attachments in parallel.
ATTACHMENT_POOL_WORKERS = 4

#: Multipart connections allowed per attachment upload. Attachments already run
#: in a thread pool and multipart defaults to 10 connections each, so without a
#: cap the product would be ATTACHMENT_POOL_WORKERS x 10 open connections. The
#: two constants are only meaningful together — keep them adjacent so a change
#: to one prompts a look at the other.
ATTACHMENT_MAX_CONCURRENCY = 3
...
class CellMeasurementClient:
    ...
    #: Timeout for uploads as (connect, read) in seconds. Passed through to the
    #: shared transport, so overriding it takes effect on both paths; on the
    #: multipart path the read timeout applies per part, not per transfer.
    ...
    def _detail_time_series(
        ...
        if include_steps:
            if have_steps:
                ...
                try:
                    steps = parquet_to_dataframe(futures["steps"].result())
        ...
        if include_time_series:
            if have_ts:
                ...
                time_series = parquet_to_dataframe(futures["ts"].result())
    ...
    def steps(self, measurement_id: str, use_cache: bool = True) -> DataFrame:
        ...
        def fetch() -> DataFrame:
            ...
            return parquet_to_dataframe(content)
    ...
    def update(self, measurement_id: str, data: dict[str, Any]) -> CellMeasurement:
        ...
        data : dict[str, Any]
            Dictionary containing the fields to update. This is the path for
            attaching/detaching a ``channel_id`` (pass ``None`` to detach), for
            setting ``end_time`` to mark a run complete, and for attaching a
            ``protocol_id`` once the protocol a run executed is known (it must
            belong to the measurement's own project).
        ...
        ------
        IonworksError
            When attaching a ``channel_id`` breaks a channel rule:
            ``BAD_REQUEST`` (400) for a cross-project channel, a
            non-time_series measurement, or a missing ``start_time``;
            ``CONFLICT`` (409) for an out-of-commission channel or a time
            span overlapping another run already on that channel.
        """
        endpoint = f"/cell_measurements/{measurement_id}"
    ...
    def attach_raw_data(
        ...
        cell_measurement_id : str
            The ID of the cell measurement to attach raw data to.
        raw_data_ids : list[str]
            IDs of the raw-data records to link to this measurement.
        """
        self.client.post(
    ...
    def detach_raw_data(self, cell_measurement_id: str, raw_data_id: str) -> None:
        ...
        ----------
        cell_measurement_id : str
            The ID of the cell measurement to detach raw data from.
        raw_data_id : str
            ID of the raw-data record to unlink from this measurement.
        """
        self.client.delete(
            f"/cell_measurements/{cell_measurement_id}/raw_data/{raw_data_id}"
        )

    def watch(self, measurement_id: str) -> None:
        """Start watching a measurement (the "watch this channel" action).

        A watch is per-user and keyed on the measurement, but reads as watching
        the channel the measurement is running on: it powers the lab "my
        channels" view (``client.lab.watched_channels``) and clears itself once
        the test finishes, so you are no longer watching a channel whose test
        has ended. Idempotent -- watching an already-watched measurement is a
        no-op.

        Parameters
        ----------
        measurement_id : str
            The ID of the (live) measurement to watch -- typically the one on
            test on a channel (``channel.measurement.id`` in the lab status).

        Raises
        ------
        IonworksError
            ``NOT_FOUND`` (404) if the measurement does not exist.
        """
        self.client.post(f"/cell_measurements/{measurement_id}/watch", {})

    def unwatch(self, measurement_id: str) -> None:
        ...
        ----------
        measurement_id : str
            The ID of the measurement to stop watching.
        """
        self.client.delete(f"/cell_measurements/{measurement_id}/watch")
    ...
    def list_raw_data(
        ...
        ----------
        cell_measurement_id : str
            The ID of the cell measurement whose raw-data links to list.
        limit : int, optional
            Maximum number of raw-data records to return per page. Defaults
            to 100.
        offset : int, optional
            Number of raw-data records to skip for pagination. Defaults to 0.
    ...
    def _upload_upload_target(
        ...
        """Upload data to an upload target, single-PUT or S3 multipart by size.

        Takes the whole ``UploadInfo`` rather than just its signed URL because
        the multipart credentials live alongside it, and a seekable handle
        rather than bytes because only a handle can take the multipart path.
        ...
        data : io.IOBase
            Seekable binary handle holding the file content.
        size_bytes : int
            Body length, used to choose between single PUT and multipart.
        content_type : str, optional
            MIME type for the upload. Defaults to "application/octet-stream".
        max_concurrency : int | None, optional
            Caps multipart connections. Callers running their own thread pool
            pass a reduced value so the product of the two stays bounded.
    ...
    @staticmethod
    def _require_upload(initiate_result: InitiateUploadResponse, filename: str):
        """Pick the named upload target out of an initiate response.

        Parameters
        ----------
        initiate_result : InitiateUploadResponse
            Response from an initiate-upload / initiate-extend call.
        filename : str
            Leaf file name to match, e.g. ``"time_series.parquet"``. Matching is
            on the leaf name, not the path, so a claim-scoped upload path still
            resolves.

        Returns
        -------
        UploadInfo
            The matching upload entry.

        Raises
        ------
        IonworksError
            If the server returned no upload for *filename*.
        """
        upload = next(
            (u for u in initiate_result.uploads if u.filename == filename), None
        )
        if upload is None:
            raise IonworksError(f"No {filename} upload URL returned from server")
        return upload

    def wait_for_processing(
        self,
        measurement_ids: str | Iterable[str],
        timeout: float = PROCESSING_TIMEOUT_SECONDS,
    ) -> dict[str, dict]:
        """Block until the server finishes deriving uploads' steps.

        Polls ``processing_status`` until every measurement is terminal. Steps
        are excluded from the reads, since the server refuses them while
        processing is in flight.

        Pass a whole batch in one call: uploads process concurrently, so the
        batch takes about as long as its slowest member and shares one
        *timeout*. Not fail-fast — every failure is reported once, together,
        when the batch settles.

        Parameters
        ----------
        measurement_ids : str | Iterable[str]
            The measurement, or measurements, to wait on.
        timeout : float, optional
            Seconds to wait before giving up on whatever is still processing.
            Shared across the whole batch. Defaults to
            ``PROCESSING_TIMEOUT_SECONDS``.

        Returns
        -------
        dict[str, dict]
            The last measurement payload read for each id, keyed by id. Returned
            so a caller that needs the settled row does not have to re-read what
            the final poll already fetched.

        Raises
        ------
        MeasurementProcessingError
            If the server rejected any upload, or if any did not finish within
            *timeout*. Inspect ``failures`` for the per-measurement reasons. A
            timeout does not cancel the work — it only stops waiting — so a
            measurement may still complete afterwards.
        """
        ids = _as_id_list(measurement_ids)
        pending = list(ids)
        failures: dict[str, str] = {}
        ...
        while pending:
            still_pending = []
            for measurement_id in pending:
                data = self.client.get(
                    f"/cell_measurements/{measurement_id}"
                    f"?include_steps=false&include_time_series=false"
                )
                settled[measurement_id] = data
                status = data.get("processing_status")
                # Measurements that never process steps (file, properties) and
                # records predating async processing report ready or nothing.
                if status is None or status == "ready":
                    continue
                if status == "failed":
                    failures[measurement_id] = (
                        data.get("processing_error")
                        or "The server could not process this upload."
                    )
                    continue
                still_pending.append(measurement_id)
            ...
            if time.monotonic() >= deadline:
                for measurement_id in pending:
                    failures[measurement_id] = (
                        f"Still processing after {timeout:.0f}s. The upload "
                        f"may yet finish — check processing_status to find out."
                    )
                break
        ...
        if failures:
            raise MeasurementProcessingError(
                _processing_failure_message(failures, len(ids)),
                failures,
            )
        return settled
    ...
    def _confirm_upload(
        ...
        wait_for_processing : bool, optional
            Block until the server has finished deriving the steps, raising if
            it could not. Defaults to True. This call returning is otherwise no
            evidence the upload was usable: the record is created before the
            file is parsed, so a file the server cannot read still yields a
            successful response, and the rejection would surface nowhere.
            Pass False when uploading a batch, then wait on all the ids at once
            with :meth:`wait_for_processing`.

        Returns
        -------
        CellMeasurementBundleResponse
            Response containing measurement fields and steps_created. Note that
            ``steps_created`` is always 0 — steps are derived after this call
            returns, so the count is not known here.

        Raises
        ------
        MeasurementProcessingError
            If ``wait_for_processing`` and the server could not process the
            upload (or did not finish in time).
        """
        ...
        bundle = CellMeasurementBundleResponse(**response_data)
        if wait_for_processing and bundle.processing_status in (
            "pending",
            "running",
        ):
            self.wait_for_processing(measurement_id)
        return bundle
    ...
    def create(
        ...
        measurement_detail : dict[str, Any]
            ...
            - measurement: dict with 'name' (required) and 'notes'. An optional
              'data_type' key routes validation: ``"ocp"`` for open-circuit
              potential data and ``"eis"`` for impedance spectra (validates the
              EIS columns and, in strict mode, the ``Z_Im`` sign and impedance
              magnitude). It is a client-side validation hint only and is not
              persisted. May also carry ``channel_id`` (the physical lab channel
              the test ran on) and ``start_time``. If ``channel_id`` is set, this
              measurement is a channel-linked run, which requires ``start_time``
              and forbids overlapping another run on the same channel — see the
              Raises note below and the ``manage-equipment`` skill.
        ...
        voltage_window : tuple[float, float], optional
            ...
            chronological order.
        skip_checks : Iterable[str], optional
            Names of strict-mode checks to skip while keeping
            ``validate_strict=True`` for everything else. Prefer this over
            disabling strict mode entirely when only a single check is a
            known false positive for the dataset. See
            :data:`ionworks.validators.STRICT_CHECK_NAMES` for valid names.
        wait_for_processing : bool, optional
            Block until the server has finished deriving the steps, raising if
            it could not. Defaults to True, because this call returning is
            otherwise no evidence the upload was usable: the record is created
            before the file is parsed, so a file the server cannot read still
            yields a successful response.

            Pass False when uploading a batch. The server processes uploads
            concurrently, so submitting all of them and then waiting on every
            id at once with :meth:`wait_for_processing` takes about as long as
            the slowest single upload, where waiting inside each call would
            take the sum.
        ...
        CellMeasurementBundleResponse
            ...
            file path. ``steps_created`` is always 0 — steps are derived after
            the call returns, so the count is not known yet.
        ...
        IonworksError
            When a ``channel_id`` in ``measurement`` breaks a channel rule:
            ``BAD_REQUEST`` (HTTP 400) if the channel is in another project,
            the measurement is not ``time_series``, or ``start_time`` is
            missing; ``CONFLICT`` (HTTP 409) if the channel is out of
            commission or the run's time span overlaps another measurement
        ...
        time_series = to_polars(measurement_detail["time_series"])
        raw_steps = measurement_detail.get("steps")
        steps_df_for_validation = (
            to_polars(raw_steps) if raw_steps is not None else None
        )
        ...
        validate_measurement_data(
            ...
            start_time=measurement_info.get("start_time"),
            end_time=measurement_info.get("end_time"),
        )
        ...
        # Step 4/5: Serialise to parquet and upload the time series.
        # `parquet_upload_body` yields a seekable handle (only a handle can
        # take the multipart path) and spills a large frame to a temporary file
        # rather than holding the parquet in RAM alongside the frame itself.
        ts_upload = self._require_upload(initiate_result, "time_series.parquet")
        with parquet_upload_body(time_series) as (body, size_bytes):
            self._upload_upload_target(ts_upload, body, size_bytes)

        # Step 6: Upload steps if provided
        steps = measurement_detail.get("steps")
        if steps is not None:
            steps_upload = self._require_upload(initiate_result, "steps.parquet")
            steps_pl = to_polars(df_to_dict_validator(steps))
            with parquet_upload_body(steps_pl) as (body, size_bytes):
                self._upload_upload_target(steps_upload, body, size_bytes)
    ...
    def extend(
        ...
        """Append a delta time series to an unfinished measurement.

        Uploads the delta parquet via signed URL, then confirms so the server
        stitches it on and recomputes steps. Client-side checks are lighter
        than :meth:`create`: empty frame, required columns, and monotonic time
        only — time need not start at zero.

        Stitching runs after confirm returns, so this waits for it by default
        (see *wait_for_processing*). A measurement must be done processing and
        may have only one extend in flight; both refusals are a ``409``, so
        retry after waiting rather than treating them as fatal.

        Parameters
        ----------
        measurement_id : str
            ID of the unfinished time_series measurement to extend.
        time_series : Any
            Delta rows as a pandas/polars DataFrame or dict. Must include
            ``Time [s]`` and ``Step count``, be non-empty, and have
            monotonically non-decreasing time.
        wait_for_processing : bool, optional
            Block until the server has stitched the delta on, and raise if it
            could not. Defaults to True. Pass False to return as soon as the
            work is queued, then wait yourself with :meth:`wait_for_processing`
            — note that a failed extend leaves the measurement readable, so it
            reports as ready and you must check ``processing_error`` to see
            whether the delta actually applied.

        Returns
        -------
        CellMeasurementBundleResponse
            Updated measurement fields. ``steps_created`` is always 0 — steps
            are recomputed after this call returns, so the count is not known
            here.

        Raises
        ------
        IonworksError
            If the delta is empty, missing required columns, or the server
            returns no extend upload URL.
        MeasurementValidationError
            If ``Time [s]`` is not monotonically non-decreasing.
        MeasurementProcessingError
            If ``wait_for_processing`` and the server could not apply the delta
            (or did not finish in time).
        """
        df = to_polars(time_series)

        if len(df) == 0:
            raise IonworksError(
                "Extend delta time series is empty. Provide at least one row to append."
            )

        required = ("Time [s]", "Step count")
        missing = [col for col in required if col not in df.columns]
        if missing:
            raise IonworksError(
                "Extend delta must include columns "
                f"{list(required)}. Missing: {missing}."
            )

        issues = validate_time_monotonic(df)
        if issues:
            raise MeasurementValidationError(issues[0].message, errors=issues)

        initiate_result = InitiateUploadResponse(
            **self.client.post(
                f"/cell_measurements/{measurement_id}/initiate-extend",
                {},
            )
        )
        extend_upload = self._require_upload(
            initiate_result, "time_series_extend.parquet"
        )
        with parquet_upload_body(df) as (body, size_bytes):
            self._upload_upload_target(extend_upload, body, size_bytes)

        response_data = self.client.post(
            f"/cell_measurements/{measurement_id}/confirm-extend",
            {},
        )
        response_data.pop("instance", None)
        bundle = CellMeasurementBundleResponse(**response_data)
        ...
        # The wait's final poll already read the settled row, so reuse it rather
        # than spending another round-trip on the identical GET.
        refreshed = self.wait_for_processing(measurement_id)[measurement_id]
        # A failed extend returns to ready, so only processing_error detects
        # it. Claiming cleared any earlier error, so this one is ours.
        error = refreshed.get("processing_error")
        if error:
            raise MeasurementProcessingError(
                _processing_failure_message({measurement_id: error}, 1),
                {measurement_id: error},
            )
        # Report the settled row, not the mid-flight one, keeping only the
        # bundle's own fields (detail carries extras).
        known = CellMeasurementBundleResponse.model_fields
        merged = {**response_data, **{k: v for k, v in refreshed.items() if k in known}}
        return CellMeasurementBundleResponse(**merged)
    ...
    def create_or_get(
        ...
        skip_checks : Iterable[str], optional
            Names of strict-mode checks to skip. Forwarded to
            :meth:`create`. Prefer skipping a specific check over
            disabling strict mode entirely.
        wait_for_processing : bool, optional
            Forwarded to :meth:`create`. Defaults to True. Pass False when
            uploading a batch and wait on the ids together afterwards with
            :meth:`wait_for_processing`. Note that a measurement returned
            because it already existed has processed long ago, so only the
            newly created ids are worth waiting on.
    ...
    def create_properties(
        ...
        *,
        ...
        end_time : str | None, optional
            ISO-formatted end time. Set once the test is complete; leave None
    ...
    def create_file(
        ...
        *,
        ...
        def _upload_one(upload_info: Any, filepath: str) -> None:
            ...
            with open(filepath, "rb") as f:
                self._upload_upload_target(
                    upload_info,
                    f,
                    size_bytes,
                    content_type=content_type,
                    max_concurrency=ATTACHMENT_MAX_CONCURRENCY,
                )

        with ThreadPoolExecutor(
            max_workers=min(len(initiate_result.uploads), ATTACHMENT_POOL_WORKERS)
        ) as pool:
            futures = [
