...
#: Bodies at or above this size prefer S3 multipart instead of a single PUT.
UPLOAD_MULTIPART_THRESHOLD_BYTES: int = 32 * 1024 * 1024
...
class _SignedPutError(IonworksError):
    """A signed-PUT failure, retaining its status for transport recovery."""
...
def upload_to_signed_url(
    ...
    try:
        ...
        raise _SignedPutError(
            f"Failed to upload to signed URL: {e}",
...
def upload_file(
    ...
    *,
    ...
    """
    can_multipart = (
        target.multipart is not None
        and hasattr(body, "read")
        and getattr(body, "seekable", lambda: False)()
    )
    start = body.tell() if can_multipart else None
    multipart_unavailable = False
    if can_multipart and size_bytes >= UPLOAD_MULTIPART_THRESHOLD_BYTES:
    ...
    except _SignedPutError as error:
        if (
            error.status_code in {504, 524}
            and can_multipart
            and not multipart_unavailable
        ):
            body.seek(start)
            try:
                _upload_multipart(
...
def _upload_multipart(
    ...
    *,
    ...
    force_multipart : bool, optional
        Uses a one-byte selector threshold so a signed-PUT timeout recovery
        cannot be routed back to PutObject by s3transfer.
    ...
    config = TransferConfig(
        multipart_threshold=1 if force_multipart else UPLOAD_MULTIPART_THRESHOLD_BYTES,
