...
def collect_all_pages(fetch_page, page_size: int = 100) -> list:
    """Page through a paginated endpoint and return every item.

    Advances ``offset`` by the number of items actually returned (not by
    ``page_size``) and stops on an empty page, so a short or empty intermediate
    page can neither skip records nor loop forever.

    Parameters
    ----------
    fetch_page : callable
        Called as ``fetch_page(limit, offset)``, returning a
        :class:`PaginatedList`.
    page_size : int, optional
        Rows to request per call. Routes cap this at 100.

    Returns
    -------
    list
        Every item across all pages, in page order.
    """
    items: list = []
    offset = 0
    while True:
...
def _parse_list_response(  # noqa: UP047
    ...
    key : str, optional
        Key holding the page of results in a paginated response. Defaults to
        ``"items"``; ``/pipelines`` nests them under ``"pipelines"`` instead.
    ...
    if isinstance(response_data, dict) and key in response_data:
        items = [model_class(**item) for item in response_data[key]]
        return PaginatedList(items=items, total=response_data.get("total", len(items)))
...
class CellMeasurement(BaseModel):
    ...
    #: Optional protocol the measurement ran against, in its own project.
    #: Preferred over the free-form ``protocol`` dict.
...
class MultipartUploadInfo(BaseModel):
    """S3 multipart credentials and target for a large direct-to-storage upload.

    Returned inside :class:`UploadInfo` only when the backend supports the
    multipart path. The SDK normally uses it at or above
    ``UPLOAD_MULTIPART_THRESHOLD_BYTES`` and PUTs smaller files to
    ``signed_url``. A 504 or 524 signed-PUT failure falls forward to multipart
    once when the body is seekable.
    """
...
class UploadInfo(BaseModel):
    ...
    token: str
    path: str
    #: Present only when the backend offers an S3 multipart target.
...
class ParsedProtocol(BaseModel):
    ...
    #: Nominal cell capacity in A.h, when the file declared one (BioLogic headers do).
