...
class Ionworks:
    ...
    def __init__(
        ...
        # Optional active organization from the IONWORKS_ORGANIZATION_ID env var.
        # When set, sent as an X-Organization-Id header so requests are scoped to
        # that org. Intended for server-side callers whose auth principal (a
        # bearer token) may span multiple orgs; with an API key the org is
        # already fixed, so this is left as an env var rather than a parameter.
        self.organization_id = os.getenv("IONWORKS_ORGANIZATION_ID")

        logger.info(
            "Ionworks client api_url=%s project_id=%s",
            self.api_url,
            self.project_id,
        )
    ...
    def resolve_measurement(
        self, cell_specification: str, cell_instance: str, measurement: str
    ) -> CellMeasurement:
        ...
        spec = self._resolve_unique_by_name(
            self.cell_spec.list(name_exact=cell_specification),
            "cell specification",
            cell_specification,
        )
