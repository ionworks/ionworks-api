...
class RawDataClient:
    ...
    def upload(
        ...
        timeout : tuple[float, float] | float | None, optional
            Overrides the default upload timeout. Accepts a ``(connect, read)``
            pair or a single value used for both. On the multipart path (see
            Notes) this timeout applies **per part**, not to the whole
            transfer. Defaults to ``None``, which falls back to the shared
            transport's default.
        ...
        ``initiate-upload`` to obtain a short-lived signed URL, uploads the
        file bytes directly to storage, then calls ``confirm-upload`` to
        create the record. The file body never passes through the API server.

        Files below 512 MiB upload in a single request. Larger files upload
        via S3 multipart instead, which requires the optional extra: install
        it with ``pip install 'ionworks-api[large-uploads]'``. Without the
        extra, files up to 1 GiB still fall back to the single request; above
        that the extra is required, since the single request cannot carry them.
        """
