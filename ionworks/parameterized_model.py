...
class ParameterizedModelClient:
    ...
    def create(self, cell_spec_id: str, data: dict[str, Any]) -> ParameterizedModel:
        ...
        data : dict[str, Any]
            ...
            ``parameters``, ``simulation_settings``.

            ``simulation_settings`` is a persistent bag of pybamm simulation
            kwargs (``var_pts`` / ``submesh_types`` / ``spatial_methods`` /
            ``solver``) that takes precedence over the base model's settings when
            this parameterized model is simulated. Pass an
            ``iws.models.SimulationSettings(...)`` object (serialized for you) or
            its config dict (see :meth:`ModelClient.create`).
        ...
        response_data = self.client.post(endpoint, normalize_simulation_settings(data))
        return ParameterizedModel(**response_data)
    ...
    def update(
        ...
        """Update a parameterized model's name, description, or simulation settings.
        ...
        data : dict[str, Any]
            ...
            ``description``, and ``simulation_settings`` (a ``SimulationSettings``
            object or its config dict; send ``null`` to clear the persisted
            settings and fall back to the base model / defaults).
        ...
        response_data = self.client.patch(endpoint, normalize_simulation_settings(data))
        return ParameterizedModel(**response_data)
