...
#: ``"arbin"`` is the .sdu dialect (MITS Pro <=7); ``"arbin_sdx"`` is .sdx
#: (MITS Pro 8+). Both are first-class -- "arbin" is not deprecated.
ConversionTarget = Literal[
    "maccor", "arbin", "arbin_sdx", "neware", "biologic_bttest", "novonix"
]
...
class ProtocolClient:
    ...
    def convert(
        ...
        verify : bool, optional
            Ask the server to round-trip the emitted file back through its
            parser and reject the conversion if the protocol no longer means the
            same thing -- a dropped loop count, goto, or safety bound. Defaults
            to False, matching the server: the checker still reports several
            benign artifacts of the Maccor, Arbin and Neware writers as
            differences, so enabling it can refuse a valid conversion to those
            targets. Reliable today for ``biologic_bttest`` and ``novonix``.
