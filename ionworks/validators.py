...
class IssueCode(StrEnum):
    ...
    TIME_MISSING_ON_NON_EIS_ROW = "time_missing_on_non_eis_row"
...
def validate_positive_current_is_discharge(  # noqa: PLR0913
    ...
    # Integrating |I| dt through a null yields NaN, which fails the per-step
    # weight guard and can leave the ECM fit singular.
    timed = ~np.isnan(time)
    if step_col and _has_column(df, step_col):
        step_values = _get_column(df, step_col)
    else:
        step_values = None
    if not timed.all():
        current, voltage, time = current[timed], voltage[timed], time[timed]
        if step_values is not None:
            step_values = step_values[timed]
...
def _is_eis_row(df: DataFrame, n_rows: int) -> np.ndarray:
    """Return a boolean mask of rows carrying an EIS frequency.

    A sweep is measured against frequency rather than the clock, so these are
    the only rows allowed to omit a time.
    ...
    df : DataFrame
        Time series data (pandas or polars).
    n_rows : int
        Length of the frame, used when no frequency column is present.

    Returns
    -------
    np.ndarray
        Boolean mask, all False when the frame has no frequency column.
    """
    if not _has_column(df, EIS_FREQ_COL):
        return np.zeros(n_rows, dtype=bool)
    freq = _get_column(df, EIS_FREQ_COL)
    return ~np.isnan(freq)


def validate_time_missing_only_on_eis_rows(
    df: DataFrame,
    time_col: str = "Time [s]",
) -> list[ValidationIssue]:
    """Validate that only EIS rows omit a time.

    An EIS sweep is recorded against frequency, not the clock, so its rows
    carry a null ``Time [s]``. Any other row missing a time is a gap in the
    measurement, not a frequency-domain record.
    ...
    untimed = np.isnan(time_data)
    offenders = np.flatnonzero(untimed & ~_is_eis_row(df, len(time_data)))
    if len(offenders) == 0:
        return []

    first_idx = int(offenders[0])
    return [
        ValidationIssue(
            IssueCode.TIME_MISSING_ON_NON_EIS_ROW,
            f"Column '{time_col}' is missing on {len(offenders)} row(s) that "
            f"carry no EIS frequency, first at index {first_idx}. Only EIS "
            f"rows, which are measured against frequency rather than time, "
            f"may omit a time.",
            payload={
                "row_index": first_idx,
                "num_rows": int(len(offenders)),
            },
        )
    ]


def _first_consecutive_timed_violation(
    df: DataFrame,
    time_col: str,
    predicate: Callable[[np.ndarray], np.ndarray],
) -> tuple[int, int, np.ndarray, np.ndarray] | None:
    """Find the first pair of consecutive timed rows failing *predicate*.

    Rows with no time are EIS sweeps, measured against frequency rather than
    the clock. They are stepped over rather than diffed through: NaN fails
    every comparison, so a gap or reversal spanning one would go unreported.
    ...
    time_col : str
        Name of the time column.
    predicate : Callable[[np.ndarray], np.ndarray]
        Maps the diffs between consecutive timed rows to a boolean mask of
        violations.

    Returns
    -------
    tuple[int, int, np.ndarray, np.ndarray] | None
        ``(prev_idx, next_idx, diffs, bad_mask)`` for the first violation,
        with indices addressing the original frame. None when the column is
        absent, fewer than two rows carry a time, or nothing violates.
    """
    if not _has_column(df, time_col):
        return None

    times = np.asarray(_get_column(df, time_col), dtype=float)
    if len(times) < 2:
        return None

    timed = np.flatnonzero(~np.isnan(times))
    if len(timed) < 2:
        return None

    diffs = np.diff(times[timed])
    bad_mask = predicate(diffs)
    if not np.any(bad_mask):
        return None

    first_bad = int(np.flatnonzero(bad_mask)[0])
    return int(timed[first_bad]), int(timed[first_bad + 1]), diffs, bad_mask
...
def validate_time_starts_at_zero(
    ...
    # ``abs(NaN) > tolerance`` is False, so a leading EIS row would pass any
    # start value.
    timed = np.flatnonzero(~np.isnan(time_data))
    if len(timed) == 0:
        return []

    first = float(time_data[timed[0]])
...
def validate_time_monotonic(
    ...
    list[ValidationIssue]
        List of validation issues. Empty if validation passes.
    """
    found = _first_consecutive_timed_violation(
        df, time_col, lambda diffs: diffs < -tolerance
    )
    if found is None:
        return []
    prev_idx, next_idx, _diffs, bad_mask = found

    time_data = _get_column(df, time_col)
    prev_t = float(time_data[prev_idx])
    next_t = float(time_data[next_idx])
    return [
        ValidationIssue(
            IssueCode.TIME_NOT_MONOTONIC,
            f"Column '{time_col}' must be monotonically non-decreasing. "
            f"At index {next_idx}: {next_t:.6f}s < "
...
def validate_time_gaps(
    ...
    """
    found = _first_consecutive_timed_violation(
        df, time_col, lambda diffs: diffs > max_gap_seconds
    )
    if found is None:
        return []
    first_idx, next_idx, diffs, bad_mask = found

    gap_seconds = float(diffs[np.flatnonzero(bad_mask)[0]])
    ...
    return [
        ValidationIssue(
            ...
            f"{next_idx} exceeds the maximum allowed gap of "
...
def validate_capacity_energy_from_current_power(
    ...
    # Integrating through a null NaNs every later row, which the comparison
    # then skips — disabling the check rather than failing it.
    timed = ~np.isnan(time)
    if not timed.all():
        df = (
            df.filter(pl.Series(timed))
            if isinstance(df, pl.DataFrame)
            else df.loc[timed].reset_index(drop=True)
        )
        time = time[timed]
        step_data = _get_column(df, step_col)
        if len(time) < 2:
...
def validate_measurement_data(
    ...
    if data_type == "ocp":
        ...
        all_errors.extend(validate_time_missing_only_on_eis_rows(df))
