...
class JobClient:
    ...
    def get_metadata(self, job_id: str) -> dict[str, Any]:
        ...
        The parsed JSON of the job's metadata storage object, including the historic
        ``validation_results`` and ``baseline_validation_results`` keys, plot config,
        and ``intermediate_results`` (see :meth:`get_parameter_trace`). Validation
        series are materialized from their per-objective parquet files for compatibility,
        so this call can be expensive. Prefer
        :meth:`Pipeline.get_element_series_channels` and
        :meth:`Pipeline.get_element_series` for bounded reads.
    ...
    def get_plot_data(
        ...
        max_points : int, optional
            Maximum points returned per trace (100-80000). Defaults to 2000.
    ...
    def get_series_channels(self, job_id: str) -> dict[str, dict[str, list[str]]]:
        """List a job's validation-series objectives and the channels each carries.

        Wraps ``GET /jobs/{job_id}/series/channels``. Names only, no samples. Call this
        before :meth:`get_series` to learn the objective names, the channel names, and
        which of the two solves each objective has.

        Parameters
        ----------
        job_id : str
            The data-fit, validation, or optimization job to describe.

        Returns
        -------
        dict[str, dict[str, list[str]]]
            ``{objective: {source: [channel, ...]}}``, where ``source`` is ``"optimal"``
            or ``"baseline"``. Only genuine per-sample channels appear, so a scalar
            recorded beside the series is not listed. Empty when the job recorded none.
        """
        return self.client.get(f"/jobs/{job_id}/series/channels")
    ...
    def get_series(
        ...
        """Get one objective's model-vs-data series, windowed and downsampled.

        Wraps ``GET /jobs/{job_id}/series``. Reads one objective's stored series rather
        than the whole job metadata, so the cost is set by the objective asked for rather
        than by the size of the fit. The window is applied before the row budget, so the
        points returned are spent on the range asked for.

        Parameters
        ----------
        job_id : str
            The job whose validation series to read.
        objective : str
            Objective name — a key of the mapping :meth:`get_series_channels` returns.
        source : str, optional
            Which solve to read: ``"optimal"`` (default) or ``"baseline"``.
        columns : list of str, optional
            Channels to return besides ``x_column``. Defaults to every channel the
            objective carries. Sent as one comma-separated parameter, so a channel whose
            name contains a comma cannot be requested.
        x_column : str, optional
            Channel plotted on x, and therefore the one windowed and binned on. Defaults
            to the objective's own x — ``"Time [s]"`` for a time-domain fit,
            ``"Frequency [Hz]"`` for an impedance one — so it rarely needs setting.
        max_points : int, optional
            Row ceiling for the response. Omit to return the window whole, which is the
            only way to be certain nothing was decimated: the completeness header the
            browser reads is not exposed here.
        x_min : float, optional
            Window lower bound, in the units of ``x_column``. Omit for the full range.
        x_max : float, optional
            Window upper bound, in the units of ``x_column``. Omit for the full range.
        ...
        ------
        IonworksError
            If ``source``, ``objective``, ``x_column``, or a requested column is not
            present on the job.
        """
        endpoint = _build_endpoint(
            f"/jobs/{job_id}/series",
            {
                "objective": objective,
                "source": source,
