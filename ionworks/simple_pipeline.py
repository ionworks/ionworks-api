...
class SimplePipelineSubmissionResponse(BaseModel):
    ...
    #: Server-derived, so the client never hard-codes the status taxonomy.
    is_terminal: bool
    is_failed: bool
    result: dict[str, Any] | None = None

    #: HTTP client backing result_object's lazy fetchers; None when unset.
    _client: Any = PrivateAttr(default=None)

    def __init__(self, **data: Any) -> None:
        """Build the response, accepting the HTTP client as a keyword argument.

        Pydantic silently drops underscore-prefixed keyword arguments, so
        ``client`` is popped here and assigned to ``_client``. Passing it at
        construction keeps a response from ever existing without it — the lazy
        fetchers no-op on a missing client, which would otherwise surface much
        later as an empty overlay.

        Parameters
        ----------
        ``**data``
            The backend's response fields, plus an optional ``client``.
        """
    ...
    @property
    def result_object(self) -> iws.BaseResults:
        """The typed ``ionworks_schema`` result decoded from :attr:`result`.

        A SimplePipeline runs a single data-fit or validation element, so its
        flat ``result`` dict is decoded directly, with the overlay/trace/posterior
        fetchers bound to this pipeline's ``job_id``.

        Returns
        -------
        ionworks_schema.BaseResults
            The typed result object. A ``BaseResults`` when ``result`` is not
            yet available (e.g. the pipeline has not completed).
        """
        from .pipeline import _coerce_element_result

        result = _coerce_element_result(
            self.name or "result", self.result, self._client, self.job_id
        )
        result.explain_no_plots(
            f"SimplePipeline {self.id} stored no model-vs-data plot data, so "
            "there is nothing to draw. A SimplePipeline data fit stores an "
            "overlay only when the pipeline includes a Validation element — add "
            "one, or run the same elements as a full Pipeline, whose data fit "
            "validates its best-fit parameters and stores the overlay itself."
        )
        return result
...
class SimplePipelineClient:
    ...
    def _build_response(
        ...
        """Build a response with the client attached so result_object's lazy
        fetchers can reach the API."""
        return SimplePipelineSubmissionResponse(**response_data, client=self.client)
    ...
    def list(
        ...
        page = _parse_list_response(response_data, SimplePipelineSubmissionResponse)
        # Listed items are pydantic-parsed, not built here, so wire the client on
        # afterwards or a completed run's result_object can't fetch its overlay.
    ...
    def wait_for_completion(
        ...
        raise_on_failure : bool, optional
            Raise :class:`IonworksError` when the pipeline ends in ``failed`` or
            ``canceled``. Defaults to True.
        ...
        IonworksError
            If the pipeline failed or was canceled and ``raise_on_failure``
            is True.
        """
        ...
        if pipeline.is_failed and raise_on_failure:
            error_msg = f"SimplePipeline {simple_pipeline_id} {pipeline.status}"
            if pipeline.error:
