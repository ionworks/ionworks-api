...
from .models import _build_endpoint
...
class SimplePipelineListResponse(BaseModel):
    """Paginated list response."""

    items: list[SimplePipelineSubmissionResponse]
...
class SimplePipelineClient:
    ...
    def create(
        ...
        ----------
        config : dict[str, Any]
            Pipeline configuration with an ``elements`` dict. May contain at most
            one ``data_fit`` or ``validation`` element.
        ...
        serialized_config = run_validators_outbound(config)
    ...
    def list(
        ...
        -------
        SimplePipelineListResponse
            Paginated list with ``items``, ``count``, and ``total``.
        ...
        return SimplePipelineListResponse(**response_data)
