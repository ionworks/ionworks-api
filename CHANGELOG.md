...
### Added
- `client.analysis.find_one(measurement_id, analysis_id=..., analysis_type=...,
  name=...)` returns the single analysis on a measurement matching every field
  given. It raises when no analysis or more than one matches, listing the
  candidates so the right selector can be copied.

### Changed
- `client.simulation.get_result()` downloads the time series and steps as
  parquet rather than one JSON body. It returns the same `SimulationResult` shape as before. A 404 can
  now also mean an older result that has not yet been converted; retry after a
  moment.

### Fixed
- Files at or above 32 MiB now prefer S3 multipart uploads when
  `ionworks-api[large-uploads]` is installed. If a signed upload receives a
  storage-edge timeout, the client makes one multipart attempt from the same
  file position.

## [0.33.0] - 2026-09-22

### Changed
- App URLs built by `client.urls` no longer carry the `/dashboard` prefix: a
  project route is now `/projects/{project_id}/...`. The old URLs redirect, but
  anything comparing against a hardcoded `/dashboard/...` string needs updating.

### Fixed
- An `extra_variables` name the model does not carry is refused when the
  simulation is submitted, rather than failing later during the run.

## [0.32.0] - 2026-09-14

### Changed
- `client.pipeline.wait_for_completion()` and
  `client.simple_pipeline.wait_for_completion()` now raise `IonworksError` on a
  cancelled pipeline as well as a failed one when `raise_on_failure=True` (the
  default). The status is carried into the message (`Pipeline <id> canceled`).
  Pass `raise_on_failure=False` to get the record back instead, as before.
- `PipelineSubmissionResponse` and `SimplePipelineSubmissionResponse` carry
  server-derived `is_terminal` and `is_failed` flags, so the client no longer
  decides which statuses are terminal. A status this SDK has never heard of
  (added server-side later) is classified correctly without an SDK upgrade.
  Both fields are required, so this version needs a backend that publishes
  them.

### Fixed
- `client.pipeline.wait_for_completion()` stopped polling only on `completed`
  or `failed`, so a cancelled pipeline kept polling until `timeout` elapsed and
  then raised `TimeoutError`. It now returns (or raises, see above) as soon as
  the pipeline is cancelled.

## [0.31.0] - 2026-09-05

### Added
- `client.material_property_dataset.to_prior()` centres a fit prior on a scalar
  already stored in a property dataset, so a measured value feeds a fit without
  ...
  A scalar carries no spread, so exactly one of `std` or `rel_std` is required.
  `distribution="lognormal"` suits a strictly positive parameter spanning orders
  of magnitude, such as a diffusivity or conductivity.
- `ParsedProtocol.nominal_capacity_ah` reports the cell rating declared in a
  protocol file's header, when the file carries one.

## [0.30.0] - 2026-08-31

### Added
- Pipeline results decode into the typed result objects from `ionworks-schema`,
  so a fitted parameter or a figure is reachable directly off the result rather
  than by fetching job metadata and parameter traces by hand:

  ```python
  result = client.pipeline.result(pipeline_id)
  fit = result.element("fit")
  fit.parameter_values["Positive particle diffusivity [m2.s-1]"]
  fit.plot_fit_results()
  fit.plot_trace()
  ```

  `PipelineResponse.element(name)` and `.results` expose the typed objects, and
  `SimplePipeline` gains `result_object`. This is additive: the raw `.result`
  and `.element_results` payloads are unchanged.
- `result.element(name).series` returns the full-resolution per-objective
  model-vs-data channels, `{objective: {channel: [values]}}`, for data-fit and
  validation elements. Use `.overlay` for the decimated data behind figures and
  `.series` when you want the raw numbers to save or analyse.
- Plotting from a result object needs matplotlib, which is not installed by
  default: `pip install 'ionworks-schema[plot]'`.
...
### Added
- `client.cell_measurement.extend(measurement_id, time_series)` appends a delta
  time series to an unfinished measurement — one whose `end_time` is not yet
  set — so a running test can be uploaded incrementally instead of re-sending
  the whole series. Pass only the new rows: their `Time [s]` must start after
  the last existing sample and the column set must match the existing series
  exactly. Steps and cycles are recomputed server-side from the stitched
  result, so never pass steps. The call waits for that processing by default;
  pass `wait_for_processing=False` to return as soon as the upload is
  confirmed.
- Uploads larger than 512 MiB now use S3 multipart, making multi-gigabyte raw
  files and time series upload reliably. This needs an optional dependency:
  install it with `pip install 'ionworks-api[large-uploads]'`. Without the
  extra, files up to 1 GiB still upload via the existing single request; above
  that the extra is required.
- `client.raw_data.create()` accepts a `timeout`, as a `(connect, read)` pair
  or a single value for both. On the multipart path the timeout applies per
  part rather than to the whole transfer.
- Measurements carry a `protocol_id`, referencing the protocol a run executed.
  Set it through `cell_measurement.create()` or `update()` like any other
  field. The protocol must belong to the measurement's own project. It is
  preferred over the free-form `protocol` dict, which stays available for
  imported data with no executable protocol behind it; setting both records
  what the run was meant to execute plus the conditions it actually ran under.
- `client.pipeline` gained `update()`, `cancel()` and `delete()`, matching
  `client.simple_pipeline`. Previously a pipeline submitted through
  `client.pipeline` could not be renamed, cancelled or removed from Python at
  all, even though the API supported all three.
- `client.pipeline.list()` accepts the full filter set the endpoint supports —
  `id`, `name`, `description`, `status`, `created_by_email`, `created_at`,
  `updated_at`, the `_gt`/`_lt` variants, `order_by`, `order` and `offset` —
  rather than only `project_id` and `limit`.

### Changed
- `job.get_plot_data()` accepts up to 80000 points per trace, raised from
  10000, so a long run can be fetched at full resolution in one call. The
  default is unchanged at 2000.
- Both `client.pipeline.list()` and `client.simple_pipeline.list()` now return
  `PaginatedList`, so the two clients agree. It behaves as a list (iteration,
  indexing, `len()`, truthiness) and carries `.items`, `.count` and `.total`, so
  loops and the three attributes keep working unchanged.
- **Behaviour change:** `client.simple_pipeline.list()` previously returned a
  Pydantic model, so pydantic-only methods on the result — `model_dump()`,
  `model_dump_json()` — no longer exist. Use `.items` and serialise those. The
  underlying `SimplePipelineListResponse` class is also gone; it was never
  exported from `ionworks`, so only code importing it from the submodule
  directly is affected. `client.pipeline.list()` gains `.total`, which it
