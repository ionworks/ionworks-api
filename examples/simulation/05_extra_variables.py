...
the anode potential [V] along with standard voltage and current plots.
"""
...
extra_var_name = "Anode potential [V]"
config = {
    "parameterized_model": {
...
if len(simulation_data.time_series) == 0:
...
plot_simulation_results(ts, title="Simulation Results with Extra Variables")

# Plot the extra variable — convert to pandas for matplotlib compatibility
df = ts.to_pandas() if hasattr(ts, "to_pandas") else ts
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(df["Time [s]"], df[extra_var_name])
ax.set_xlabel("Time [s]")
ax.set_ylabel(extra_var_name)
ax.set_title("Anode Potential")
ax.grid(True)
plt.tight_layout()
plt.show()
